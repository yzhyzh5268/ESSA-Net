...
class ProfileModels:
    ...
    Attributes:
        ...
        img_size (int): Image size used in the models. Default is 640.
    ...
    def __init__(
        ...
        Args:
            ...
            img_size (int, optional): Size of the image used during profiling. Default is 640.
            ...
            if file.suffix in {".pt", ".yaml", ".yml"}:
                ...
                if self.trt and self.device.type != "cpu" and not engine_file.is_file():
                    engine_file = model.export(
                        format="engine", half=self.half, imgsz=self.img_size, device=self.device, verbose=False
                    )
                onnx_file = model.export(
                    format="onnx", half=self.half, imgsz=self.img_size, simplify=True, device=self.device, verbose=False
                )
    ...
    def profile_tensorrt_model(self, engine_file: str, eps: float = 1e-3):
        ...
        input_data = np.random.rand(self.img_size, self.img_size, 3).astype(np.float32)  # must be FP32
        ...
        for _ in range(3):
            ...
            for _ in range(self.num_warmup_runs):
                model(input_data, imgsz=self.img_size, verbose=False)
        ...
        for _ in TQDM(range(num_runs), desc=engine_file):
            results = model(input_data, imgsz=self.img_size, verbose=False)
