...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
    ...
    if xywh:  # transform from xywh to xyxy
        ...
        b1_x1, b1_y1, b1_x2, b1_y2 = box1.chunk(4, -1)#xyxy
        b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
        w1, h1 = b1_x2 - b1_x1, b1_y2 - b1_y1 + eps
        w2, h2 = b2_x2 - b2_x1, b2_y2 - b2_y1 + eps
    ...
    if CIoU or DIoU or GIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width 最小的包围框的宽
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height 最小包围框的高
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            c2 = cw.pow(2) + ch.pow(2) + eps  # convex diagonal squared 最小包围框的对角线的平方
            rho2 = (
                (b2_x1 + b2_x2 - b1_x1 - b1_x2).pow(2) + (b2_y1 + b2_y2 - b1_y1 - b1_y2).pow(2)
            ) / 4  # center dist**2  中心点距离的平方 (((x1+x2)/2)-((x1+x2)/2))
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                v = (4 / math.pi**2) * ((w2 / h2).atan() - (w1 / h1).atan()).pow(2)#4/pi2  *(arctanw2/h2-arctanw1/h1)2
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))#v/(v+1-iou)
                return iou - (rho2 / c2 + v * alpha)  # CIoU  iou -中心点距离/最小包围框对角线的平方+v*alpha
